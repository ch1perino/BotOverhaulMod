	...
	if (!g_bSuddenDeathMode && !g_bMVM || !g_bSuddenDeathMode && g_bMVM == true && team == 2)
	{
		...
		switch (class)
		{
			...
			{				
				...
				else if (StrContains( currentMap, "pl_snowycoast" , false) != -1)
				{
					...
					{ 
						...
						// Addconds
						TF2_AddCondition(client, TFCond_Gas, TFCondDuration_Infinite);
						TF2_AddCondition(client, TFCond_CritCola, TFCondDuration_Infinite); //mini crits
			...
			case TFClass_Sniper:
			{
				...
				{
					// Pack #2 Infected Bot
					if(GetEntProp(client, Prop_Send, "m_nBotSkill") == 2 && team == 2)
					{   	
						// Name
						//char name[13] = "Infected";
						//SetClientInfo(client, "name", name);
			...
			{		
				...
				else if (StrContains( currentMap, "pl_snowycoast" , false) != -1)
				{
					...
					{   	
						...
						CreateWeapon(client, "tf_weapon_rocketlauncher", 513, 10);
						CreateWeapon(client, "tf_weapon_shotgun", 1153, 10);
						CreateWeapon(client, "tf_weapon_shovel", 775, 10);
			...
			case TFClass_DemoMan:
			{
				if (StrContains( currentMap, "pl_snowycoast" , false) != -1)
				{
					...
					{   	
						// Name
						char name[13] = "Desert Traveler";
						SetClientInfo(client, "name", name);
							
						// Cosmetics
						CreateWWeapon(client, "tf_wearable", 306); // Scotch Bonnet
						CreateWWeapon(client, "tf_wearable", 30363); // Juggernaut Jacket
						CreateWWeapon(client, "tf_wearable", 771); // Liquor Locker
					}
				}
			}
			case TFClass_Medic:
			...
			{					
				...
				else if (StrContains( currentMap, "pl_snowycoast" , false) != -1)
				{
					...
					{   	
						// Name
						char name[13] = "Disgruntled Randomizer Player";
						SetClientInfo(client, "name", name);
						...
						// Cosmetics
						CreateWWeapon(client, "tf_wearable", 613); // Gym Rat
					}
					...
					{   
						...
						// Cosmetics
						CreateWWeapon(client, "tf_wearable", 991); // Hunger Force
						CreateWWeapon(client, "tf_wearable", 985); // Hockey Hair
						CreateWWeapon(client, "tf_wearable", 30280); // The Monstrous Mandible
						CreateWWeapon(client, "tf_wearable", 30653); // Sucker Slug
						CreateWWeapon(client, "tf_wearable", 744); // Pyrovision Goggles
