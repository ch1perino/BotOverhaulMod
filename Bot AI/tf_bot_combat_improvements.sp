...
{
	// Showin added this from bot_ai to improve performance.
	for(int i=1;i<=MaxClients;i++)
	{
		if (IsValidEntity(i) && IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			// If bot is near a real player then use ai tweaks.
	...
	if(IsValidClient(client) && IsFakeClient(client) && IsPlayerAlive(client) && ImproveAI[client])
	{
		float clientEyes[3];
		float targetEyes[3];
		float targetEyes2[3];
		...
		if(IsValidClient(Ent))
		{
			...
			// Sentry Gun Checker
			// Showin added this code to make bots focus on sentryguns if needed!
			...
			// Handled elsewhere in bot overhaul (also wouldn't this be better as part of the demo code from before that handles the stickies?)
			...
			if(class == TFClass_Soldier && !IsPointVisible(clientEyes, sentrygunlocation)) // Soldier bots aim at the feet.
			{
			...
			// This part contained aim assist that showin removed. The bots already aim decently on their own if the difficulty is high enough.
			// This sort of breaks the point of the difficulty levels which mainly effects things such as this.
		...
		{	
			...
			if(class != TFClass_Spy && class != TFClass_Engineer && class != TFClass_Sniper && class != TFClass_Medic && !IsWeaponSlotActive(client, 2) && GetClientButtons(client) & 1)
			{	
	...
	// Showin removed. Bot Overhaul handles it.
